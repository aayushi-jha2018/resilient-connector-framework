...
The first version of `retry_with_backoff` retried on a bare `except Exception`, which meant a `TypeError` from a genuine bug in a connector (like passing the wrong argument type) got silently retried three times and then reported as a generic "failed after 3 attempts" instead of surfacing the actual programming error immediately. Fixed by adding the `exceptions` parameter so callers can scope retries to the specific failure types they actually expect from a flaky source (timeouts, connection errors), and let anything else fail fast. `test_only_catches_configured_exception_types` is the regression test for this.
